...
function QuadrantLabel({ klass, lang, className }: { klass: MenuClass; lang: 'en' | 'he'; className: string }) {
  ...
  const Icon = meta.icon;
  return (
    <span className={`absolute inline-flex items-center gap-1.5 text-[9px] tracking-[0.3em] uppercase ${className}`} style={{ color: `${meta.color}cc`, fontFamily: sans }}>
  ...
  return (
    ...
    >
      ...
      {isHovered && (
        ...
        >
          <span
            className="mb-1.5 block truncate text-[13px] leading-tight text-white"
            style={{ fontFamily: serif, fontWeight: 700 }}
          >
            {item.title[lang]}
          </span>
          <span
            className="mb-2 block text-[9px] uppercase tracking-[0.2em]"
...
function PopStat({ v, l, accent }: { v: string; l: string; accent?: string }) {
  return (
    <span className="block">
      <span className="block text-[13px]" style={{ color: accent ?? 'rgba(255,255,255,0.9)', fontFamily: serif, fontWeight: 700 }}>{v}</span>
      <span className="block text-[8px] tracking-wide text-white/40" style={{ fontFamily: sans }}>{l}</span>
    </span>
  );
}
