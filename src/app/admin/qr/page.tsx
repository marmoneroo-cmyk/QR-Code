...
export default function QrAdminPage() {
  ...
  return (
    <AdminShell
      ...
      eyebrowHe="קודי QR"
      active="/admin/qr"
      subtitle={`Download each QR or print the whole sheet on tag stock. A scan jumps straight to the ${arMode ? 'AR view (camera required).' : '3D breakdown experience.'}`}
      subtitleHe={`הורד כל קוד QR או הדפס את הגיליון כולו. סריקה קופצת ישר ל${arMode ? 'תצוגת AR (נדרשת מצלמה).' : 'חוויית הפירוק התלת-ממדית.'}`}
      actions={
        <>
          <button
            type="button"
            ...
            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full border border-white/15 bg-white/[0.03] text-white/70 hover:text-amber-100 hover:border-amber-200/50 transition-colors text-[10px] tracking-[0.3em] uppercase disabled:opacity-40"
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-6 py-2.5 rounded-full bg-amber-200 text-black hover:bg-amber-100 transition-transform hover:scale-[1.03] text-[11px] tracking-[0.25em] uppercase"
        ...
        {cards.map((card) => {
          ...
          return (
            <div
              ...
              className="group relative overflow-hidden rounded-[1.75rem] border border-white/10 bg-gradient-to-b from-white/[0.04] to-black p-6 flex flex-col items-center text-center transition-colors hover:border-amber-200/40 print:break-inside-avoid print:border-amber-700/40 print:bg-white"
