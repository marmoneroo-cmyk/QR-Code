...
- **Dedup finished** — every `₪` amount now uses the shared `formatILS` (was 12 copies); `formatILS` + `confidenceBucket` locked with tests.
- **Business Brain** — the live opportunity engine's 5 decision rules no longer hide magic numbers (`openRate>=0.4`, `intentRate<0.15`, …); all 9 cut-points centralized into `OPPORTUNITY_THRESHOLDS` in `menu-intel/thresholds.ts` (values verbatim; `build.test.ts` green). RLS migration `0013` (`(select auth.uid())` InitPlan optimization, 15 policies) authored.
- **API validators tested + a bug fixed** — the write-body validators (experience config → public menu, promotion fields, sales rows) were extracted to pure modules `src/lib/{experience,promotions,sales}/validate.ts` and covered with **78 tests**; those tests surfaced a real hole — `checkValue(NaN)` passed (NaN is `typeof number`, `NaN<0` is false), so a NaN discount could reach the DB — now rejected via `Number.isFinite`.
- **Adversarial security review of the new surface → 3 fixes** — (1) `/api/track` trusted the client `Content-Length`; a chunked body (header omitted) set it to 0 and skipped the cap → unbounded parse on a PUBLIC endpoint. New pure `src/lib/net/bounded.ts` `readCappedText()` enforces the cap **while reading**; track + the scraper both use it (the scraper's `res.text()` was uncapped → memory exhaustion, now 8 MB). (2) Validators gained size/range bounds (negative sales, unbounded badge labels/promo names/slugs re-served to the anonymous menu). Review confirmed CLEAN: `/mine` routes, `/api/health`, migrations `0012`/`0013`, `ip.ts` obfuscated-IP handling, slug→filename paths. **309 tests total.**

**Filed for follow-up (genuinely need DB access / product decision):** SSRF DNS-rebind IP-pinning + durable `/api/track` rate limiting + the same body-cap as defense-in-depth on auth-gated write routes (lower severity — authenticated); the SQL scale fix — aggregation views/RPCs so the 9 analytics sites stop reducing ≤50k raw rows in JS per request — plus sales uniqueness (need Supabase apply; migrations `0012` index + `0013` RLS both **authored**, ready to apply); unifying the orphaned `diagnoseFunnel` "brain" + 3 parallel recommendation engines / their weight tables (product decision — reshapes owner output, and `TYPE_WEIGHT` is `OpportunityType`-coupled so it can't move without that call); isomorphic-logger consolidation.
...
### Security — Session-scoped admin reads (pre-tenant-#2)
- The admin home/promotions/experience screens read via the PUBLIC `?restaurant=diner` GET — harmless while Diner is the only tenant, but a second tenant's operator would see Diner's promotions/experience config. Added guarded `/api/promotions/mine` + `/api/experience/mine` (`requireSession` → session slug) and repointed the 3 admin reads; the public `?restaurant=` GETs stay unchanged so the anonymous diner menu pays **no** auth round-trip. Verified: anonymous `/mine` → 401, public promotions still 200. (Audit also confirmed `/api/sales` was already session-scoped — no revenue leak.)

### Changed — Business-Brain deduplication (every AI module shares one brain)
- The saturating sample-confidence curve `n/(n+60)` was copy-pasted in `funnel.ts` (diagnosis) and `actions.ts` (owner action confidence) → extracted to `src/lib/menu-intel/sample.ts` (`sampleConfidence`), so tuning it can't let the two surfaces drift. `median` (3 copies: analytics/opportunities/potential) and `clamp01` (2 copies) → `src/lib/math.ts`. Behavior identical (all prior tests pass); +9 tests pin the curves.

**Verification:** `tsc` + **205 tests** + `next build` (57 pages) green; shipped in **fourteen verified deploys**. Live checks: public menu 200 + no console errors, auth works, AdminTabs ARIA correct in-browser, launcher focus+Escape confirmed, security headers present, same-origin tracking still 200, readiness caveat renders on engine surfaces, branded 404 renders, `/api/health` 200 db:ok, anonymous `/mine` → 401. Parked: A5 secret rotation + durable `/api/track` rate limiting + Pollinations server-proxy (need dashboard/KV — filed as a follow-up task), a global text-contrast bump (aesthetic review), Epic F F2–F3, H-B (wire AI to real data).
