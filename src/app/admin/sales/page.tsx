            ...
            {row.map((d) => {
              ...
              return (
                ...
                >
                  ...
                  />
                  <p
                    className="relative truncate text-13 leading-tight text-white/90 font-serif"
                    style={{ fontStyle: isHe ? 'normal' : 'italic', fontWeight: 600 }}
                  >
                    {d.title}
                  </p>
                  <div className="relative mt-1 flex items-baseline justify-between gap-1.5">
                    <span className="text-15 font-bold leading-none tabular-nums font-serif" style={{ color: d.accent }}>
                      {formatIls(d.revenue)}
                    </span>
                    <span className="shrink-0 text-10 text-white/70 tabular-nums font-sans">
                      {sharePct}%
                    </span>
                  </div>
                </div>
      ...
      <div className="flex flex-col gap-10" dir={isHe ? 'rtl' : 'ltr'}>
        ...
        <div className="grid gap-8 lg:grid-cols-5">
          ...
          <GlassCard static className="lg:col-span-2 p-6 flex flex-col gap-4 h-fit">
            ...
            <p className="text-white/45 text-xs -mt-2 font-sans">
              {t('One row per item:', 'שורה לכל פריט:')}{' '}
              <code className="text-amber-200/80">slug,units,revenue</code>
            </p>
            <div className="grid grid-cols-2 gap-3">
              <label className="text-white/50 text-11 flex flex-col gap-1.5 font-sans">
                {t('From', 'מתאריך')}
            ...
            >
              ...
              {dragging && (
                <div className="pointer-events-none absolute inset-0 grid place-items-center rounded-2xl bg-amber-300/10 backdrop-blur-[1px]">
                  <span className="inline-flex items-center gap-2 rounded-full border border-amber-300/50 bg-black/60 px-4 py-2 text-amber-200 text-11 tracking-[0.18em] uppercase font-sans">
            ...
            <p className="text-white/70 text-11 -mt-1 inline-flex items-center gap-1.5 font-sans">
            ...
            <div className="flex items-center justify-between gap-2 text-11 font-sans">
            ...
            <button
              ...
              onClick={submit}
              disabled={saving}
              className="mt-1 inline-flex items-center justify-center gap-2 px-5 py-3 rounded-full bg-amber-300 text-black text-11 tracking-[0.25em] uppercase font-semibold transition-transform hover:scale-[1.02] disabled:opacity-50 disabled:hover:scale-100 font-sans"
            >
              <UploadCloud size={14} strokeWidth={2.2} />
              {saving ? t('Importing…', 'מייבא…') : t('Import sales', 'ייבא מכירות')}
            </button>
            {msg && <p role="status" aria-live="polite" className="text-amber-200/80 text-xs text-center font-sans">{msg}</p>}
          </GlassCard>
                    ...
                    >
                      {c ? (
                        ...
                        <div className="w-full h-32 mb-3 rounded-xl border border-dashed border-white/10 grid place-items-center">
                          <span className="text-white/70 text-10 tracking-wide font-sans">{t('no image', 'אין תמונה')}</span>
                        </div>
                      )}
                      <p className="text-white/90 text-15 text-center leading-tight font-serif" style={{ fontStyle: isHe ? 'normal' : 'italic', fontWeight: 600 }}>
                        {title}
                      </p>
                      <div className="mt-3 flex items-baseline justify-between gap-2 font-sans">
                        <span className="text-white/45 text-11">{s.units} {t('units', 'יח׳')}</span>
                        <span className="text-17 tabular-nums font-serif" style={{ color: accent, fontWeight: 700 }}>{ils(s.revenue)}</span>
                      </div>
